...
## Follow-up: fully reverted on 2026-07-25

This spec's outcome was reverted in both directions on the same day: **oxlint
and oxfmt were both removed, and ESLint 10 now owns linting *and* formatting**
through [ESLint Stylistic](https://eslint.style). `.oxlintrc.json`,
`.oxfmtrc.json`, the `lint:ox` script, and the `oxfmt` / `oxlint` /
`oxlint-tsgolint` dependencies are all gone. `pnpm run format` is now
`eslint --fix .`, and `lint` is `lint:eslint && lint:md && lint:types`.

Rationale: `.oxlintrc.json` was generated by `@oxlint/migrate` from
`eslint.config.ts`, so it was a strict subset of the ESLint config. Every rule
it enabled — for `backend/src`, `common/src`, `extension/src`, tests, and
`scripts/` — had an enabled ESLint equivalent, verified by comparing the merged
oxlint rule set against `eslint --print-config` output per file. ESLint also
kept rules oxlint never had: `local/no-plain-block-comments` (the original
blocker for Task 7), `jsdoc/require-jsdoc`, `jsdoc/require-description`,
`jsdoc/multiline-blocks`, and `padded-blocks`.

The full gate (`format:check`, `lint:eslint`, `lint:md`, `lint:types`) ran in
about 12s, so the second linter was not buying meaningful speed either.

### Known trade-offs of dropping oxfmt

Stylistic is a rule set, not a formatter, which costs three things the
formatter provided:

1. **No reflow.** Nothing re-wraps long lines. 80 columns is now a convention
   to write to; `@stylistic/max-len` errors only past 100, because oxfmt itself
   treated `printWidth: 80` as a soft target and left 25 unbreakable spans
   above it (long member chains, `(typeof X)[keyof typeof X]`, nested JSX).
2. **Only JS/TS/JSX are formatted.** Markdown, JSON, and YAML lost automatic
   formatting; `lint:md` still lints Markdown.
3. **Rules that cannot agree with the previous output are off**, documented
   inline in `eslint.config.ts`: `indent-binary-ops` and `multiline-ternary`,
   plus `indent` exemptions for union/intersection type members. The migration
   otherwise re-indented ternary bodies repo-wide (prettier aligned them +2,
   `@stylistic/indent` aligns them +4).
