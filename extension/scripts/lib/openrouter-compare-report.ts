...
function parseAlignedMetric(
  ...
  return {
    id,
    humanStartSec: expectFiniteNumber(humanStartSec, `${path}.humanStartSec`),
    ...
    predEndAssumed,
    startDeltaSec: expectFiniteNumber(startDeltaSec, `${path}.startDeltaSec`),
...
function parseCompareRow(value: unknown, index: number): CompareRow {
  ...
  if (responseModel !== undefined && typeof responseModel !== 'string') {
    throw new Error(`rows[${String(index)}].responseModel must be a string`);
  }
...
function buildUsageMainLabel(usage: CompareUsage | undefined): string {
  ...
  return `${formatCompactInteger(usage.promptTokens)}/` +
...
function buildTimelineNote(item: RankedRow): string {
  return joinDefined([
    item.summary.avgIou !== undefined
      ...
      : undefined,
    item.cost.effectiveCost !== undefined ? buildCostDisplay(item) : undefined,
...
function renderHighlights(
  ...
  const cheapest = [...rankedRows]
    .filter(
      (item) => item.row.ok && item.cost.effectiveCost !== undefined,
    )
    .sort((left, right) => {
      const byCost = compareOptionalAsc(
        left.cost.effectiveCost,
  ...
  return [
    bestOverlap === undefined
      ? renderStatCard('Best overlap', 'n/a', 'No human reference metrics')
      : renderStatCard(
          'Best overlap',
    ...
    fastest === undefined
      ? renderStatCard('Fastest response', 'n/a', 'No successful model call')
      : renderStatCard(
          'Fastest response',
          fastest.row.model,
          joinDefined([
    ...
    cheapest === undefined
      ? renderStatCard('Cheapest response', 'n/a', 'No cost data in this log')
      : renderStatCard(
          'Cheapest response',
          cheapest.row.model,
          joinDefined([
    ...
    tightestStart === undefined
      ? renderStatCard('Smallest start drift', 'n/a', 'No human reference')
      : renderStatCard(
          ...
          note:
            report.firstRunVsHuman === undefined
              ...
              : `avg IoU ${formatPercent(
                  summarizeMetrics(report.firstRunVsHuman).avgIou ?? 0,
                )}`,
...
function renderFocusCard(
  ...
  if (baselineBlock !== undefined) {
    staticLanes.push({
      ...
      segments: [
        {
          startSec: baselineMetric?.predStartSec ?? baselineBlock.startSec,
          endSec:
      ...
      segments: [
        {
          ...
          endSec: metric?.predEndSec ?? rangeEnd(block.startSec, block.endSec),
