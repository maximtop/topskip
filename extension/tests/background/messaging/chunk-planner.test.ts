...
import { MAX_CHUNKS_PER_VIDEO } from '@/background/messaging/chunk-plan-config';
...
describe('ChunkPlanner.buildChunkPlan', () => {
    it('returns no chunks for empty merged text', () => {
        const plan = ChunkPlanner.buildChunkPlan('', 500);
    ...
    it('returns no chunks when budget is non-positive', () => {
        const plan = ChunkPlanner.buildChunkPlan('[0] hi', 0);
        expect(plan.chunks).toHaveLength(0);
    });

    it('returns no chunks when there are no timed lines', () => {
        const plan = ChunkPlanner.buildChunkPlan('no bracket lines here', 100);
        expect(plan.chunks).toHaveLength(0);
    });

    it('fits the full transcript in one chunk when budget is large', () => {
        const merged = ['[0] a', '[10] b', '[20] c'].join('\n');
        const plan = ChunkPlanner.buildChunkPlan(merged, 10_000);
    ...
    it('is deterministic for the same inputs', () => {
        const merged = makeTimedTranscript(12, 2, 12);
        const budget = 80;
        const a = ChunkPlanner.buildChunkPlan(merged, budget);
        const b = ChunkPlanner.buildChunkPlan(merged, budget);
        expect(
            a.chunks.map((c) => ({
                startSec: c.startSec,
                endSec: c.endSec,
    ...
    it('produces multiple newline-aligned chunks when budget is tight', () => {
        const merged = makeTimedTranscript(24, 1, 20);
        const plan = ChunkPlanner.buildChunkPlan(merged, 55);
    ...
    it('caps at MAX_CHUNKS_PER_VIDEO and sets partial coverage', () => {
        const merged = makeTimedTranscript(200, 5, 22);
        const plan = ChunkPlanner.buildChunkPlan(merged, 48);
        expect(plan.chunks.length).toBe(MAX_CHUNKS_PER_VIDEO);
