...
import type { LlmProviderAdapter,
  AnalyzeTranscriptResult } from
...
const baseSender = (
  tabId = 42,
): Runtime.MessageSender => ({ tab: { id: tabId } }) as Runtime.MessageSender;
...
type AnalyzeFnParams = Parameters<
  LlmProviderAdapter['analyzeTranscript']
>[0];
...
describe('PromoAnalysis — adapter routing', () => {
  ...
  describe('onCaptionsReady routes through adapter', () => {
    ...
    it('routes to the provider from prefs on each run', () => {
      ...
      PromoAnalysis.onCaptionsReady(baseSender(), basePayload('chromeVid'));

      return vi.waitFor(() => {
        expect(registryMocks.get)
          .toHaveBeenCalledWith('chrome-prompt-api');
        expect(chromeAnalyze)
          .toHaveBeenCalled();
      });
    });
    ...
    it('returns not_configured when registry.get returns undefined',
      () => {
        ...
        return vi.waitFor(() => {
          ...
          expect(detectionStoreMocks.set)
    ...
    it('returns not_configured when adapter is unavailable', () => {
      const avail = vi.fn().mockResolvedValue(
    ...
    it(
      'aborts inflight analysis when the active provider changes',
      async () => {
      ...
      PromoAnalysis.onCaptionsReady(baseSender(7), basePayload('abortVid'));
      ...
      const result = await ProviderRuntimeMessages.handle(
        {
      ...
      expect(result)
        .toEqual({ ok: true });
      expect(capturedSignal?.aborted)
        .toBe(true);
      expect(prefsMocks.save)
        .toHaveBeenCalledWith({
          enabled: true,
          providerId: 'chrome-prompt-api',
        });
      expect(prefsBroadcastMocks.sendUpdatedToAllTabs)
        .toHaveBeenCalled();
      expect(prefsPortHubMocks.broadcastPrefsUpdate)
        .toHaveBeenCalledWith({
          enabled: true,
          providerId: 'chrome-prompt-api',
        });
