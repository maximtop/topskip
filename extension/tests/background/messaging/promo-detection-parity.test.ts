...
import { PromoAnalysis } from
  '@/background/messaging/promo-analysis';
import { PromoDetectionStore } from
  '@/background/promo-detection-store';
import { PromoDetectionRuntimeMessages } from
  '@/background/messaging/promo-detection-runtime-messages';
import {
  STORAGE_KEY_PREFS,
...
describe(
  ...
  () => {
    ...
    const expectedBlocks: PromoBlock[] = [
      { startSec: 30, endSec: 90, confidence: 'high' },
      { startSec: 200, endSec: 240, confidence: 'medium' },
    ];
    ...
    it(
      'stores and sends the same blocks to both surfaces',
      async () => {
        // Trigger the analysis pipeline
        PromoAnalysis.onCaptionsReady(
          { tab: { id: TAB_ID } } as never,
          {
            ok: true,
            videoId: VIDEO_ID,
            languageCode: 'en',
        ...
        // 1. PromoDetectionStore has the blocks
        ...
        const response =
          await PromoDetectionRuntimeMessages.handle(
        ...
        // 4. Verify referential identity: the blocks object
        //    sent to content is the SAME reference stored
        //    in the store (proving single source)
        const sentMsg = tabsSendMessage.mock.calls[0][1] as {
          promoBlocks: PromoBlock[];
        };
        expect(sentMsg.promoBlocks).toBe(expectedBlocks);
      },
    );
  },
);
