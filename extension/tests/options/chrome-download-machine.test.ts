...
function installLanguageModelMock(): void {
  ...
  const mockLM = {
    ...
    create: vi.fn(
      (opts?: { monitor?: (m: unknown) => void; signal?: AbortSignal }) => {
...
describe('chromeDownloadMachine', () => {
  ...
  it('transitions to downloadable when availability is' +
  ' downloadable', async () => {
    availabilityResult = 'downloadable';
  ...
  it('transitions to unavailable when availability is' +
  ' unavailable', async () => {
  ...
  it('transitions to downloading when availability is' +
  ' downloading', async () => {
  ...
  it('tracks progress from monitor events and marks' +
  ' extracting at 100%', async () => {
    availabilityResult = 'downloadable';
    createBehavior = 'hang';
