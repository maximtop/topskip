...
describe('openrouter-model-presets', () => {
    describe('isValidOpenRouterModelSlug', () => {
        ...
        it('accepts valid owner/model slugs', () => {
            expect(isValidOpenRouterModelSlug('google/gemini-2.5-flash')).toBe(
                true,
            );
            expect(isValidOpenRouterModelSlug('openai/gpt-4o')).toBe(true);
            expect(isValidOpenRouterModelSlug('meta-llama/llama-3-8b')).toBe(
                true,
            );
            expect(isValidOpenRouterModelSlug('org_name/model-name_1')).toBe(
                true,
            );
        });
    });
